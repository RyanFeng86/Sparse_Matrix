...
#Souvik: Added the tiny_imagenet_dataloader function, change the data_dir destination as necessary
def get_tiny_imagenet_dataloaders(args, validation_split=0.0):
        ...
        if validation_split > 0.0:
            split = int(np.floor((1.0-validation_split) * len(image_datasets['train'])))
            ...
            train_loader = torch.utils.data.DataLoader(image_datasets['train'], batch_size=100, \
...
def get_cifar100_dataloaders(args, validation_split=0.0):
    ...
    valid_loader = None
    train_loader = torch.utils.data.DataLoader(train, batch_size = args.batch_size, shuffle = True, pin_memory=True)
